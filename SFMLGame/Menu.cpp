...
Menu::Menu(float widht, float height) {
    ...
    menu[0].setPosition(sf::Vector2f((widht/2)-32 ,-(sizeSelectedItem-sizeNotSelectedItem)+32+height/(MAX_NUMBER_ITEMS+1)*1));
    menu[0].setCharacterSize(sizeSelectedItem);

    menu[1].setFont(font);
    menu[1].setFillColor(sf::Color::Black);
    menu[1].setString("Tutorial");
    menu[1].setPosition(sf::Vector2f((widht/2)-32,32+height/(MAX_NUMBER_ITEMS+1)*2));
    menu[1].setCharacterSize(sizeNotSelectedItem);

    menu[2].setFont(font);
    menu[2].setFillColor(sf::Color::Black);
    menu[2].setString("Exit");
    menu[2].setPosition(sf::Vector2f((widht/2)-32,32+height/(MAX_NUMBER_ITEMS+1)*3));
    menu[2].setCharacterSize(sizeNotSelectedItem);

    selectedItemIndex=0;

    texture.loadFromFile("../img/Player/Bow_Player.png");
    ...
    sprite.setPosition((widht/2)-128,12+ height/(MAX_NUMBER_ITEMS+1)*1);

    title.setFont(font);
    title.setFillColor(sf::Color::Black);
    title.setString("The road of Knght");
    title.setPosition(widht/2 -112,height/10);
    ...
    titleSprite1.setPosition(widht/3 -32,-32+ height/10);
    titleSprite2.setPosition((widht-widht/3)-48,-32+height/10);

    play=false;
    tutorial=false;

    textureBackground.loadFromFile("../img/sfondo.jpg");
